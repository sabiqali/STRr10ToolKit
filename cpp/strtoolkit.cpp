...
struct per_read_struct {
    ...
    uint8_t region_end;
    uint8_t region_ref_start;
    uint8_t region_ref_end;
    std::string motif;
    int haplotype;
    int size;
    ...
    for (auto chr: chr_list) {
        ...
        while(upper_limit <= ref_chr_size(chr)) {
           ...
           //TODO::we need to check for overlapping reads on multiple windows. if this is the case, we need to dynamically increase window size in that case.
            while (sam_itr_next(fp, itr, b) >= 0) {
                ...
                uint8_t read_pos_counter = 0;
                uint8_t ref_pos_counter = 0;
                ...
                for (k = 0; k < b->core.n_cigar; ++k) {
                    ...
                    uint8_t l = bam_cigar_oplen(cigar[k]);
                    switch(op) {
                        case BAM_CMATCH:
                            //printf("M");
                            read_pos_counter += l;
            ...
            if(max_read_support >= opt::min_read_support) {
                ...
                uint8_t mean_ref_start = 0;
                uint8_t mean_ref_end = 0;
                int read_count = 0;
		std::vector<std::string> all_motifs;
		    ...
		    all_motifs.push_back(individual_read.motif);
                ...
                mean_ref_end = (uint8_t)(mean_ref_end/read_count);
                mean_ref_start = (uint8_t)(mean_ref_start/read_count);
